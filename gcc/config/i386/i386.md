...
(define_insn "ashlsi3"
  [(set (match_operand:SI 0 "nonimmediate_operand" "=rm")
        (ashift:SI
             (match_operand:SI  1 "register_operand" "0")
             (match_operand:SI  2 "register_operand" "cU")
        )
   )
   (clobber (reg:CC O386_EFLAGS))
  ]
...
;    }
;    //if (MEM_P (operands[0]) && !o386_valid_address(SImode, XEXP(operands[0],0), false))
;    //  operands[0] = o386_legitimize_address (XEXP(operands[0],0), operands[0], SImode);
;    //else if (MEM_P (operands[1]) && !o386_valid_address(SImode, XEXP(operands[1],0), false))
;    //  operands[1] = o386_legitimize_address (XEXP(operands[1],0), operands[1], SImode);
;  }
;})
...
(define_insn "*mov<mode>"
  [(set  (match_operand:AMODE 0 "general_operand" "=r, m"  )
         (match_operand:AMODE 1 "general_operand" "rCm,rC" )
  )]
  "(nonmemory_operand (operands[0], <MODE>mode) || nonmemory_operand (operands[1], <MODE>mode))"
{
  switch (<MODE>mode) {
  case QImode: return "movb %b1, %b0";
  case HImode: return "movw %w1, %w0";
  case SImode: return "movl %1, %0";
...
(define_insn "*reg_compare"
  [(set (reg:CC O386_EFLAGS)
        (compare:CC (match_operand:SI 0 "general_operand" "rCm")
                    (match_operand:SI 1 "general_operand" "rCm")
        )
  )
  ]
 ""
